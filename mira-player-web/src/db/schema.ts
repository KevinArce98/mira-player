import type { WebDatabase } from '@/db';
...
export async function runMigrations(db: WebDatabase): Promise<void> {
  ...
  for (let version = current; version < MIGRATIONS.length; version++) {
    ...
    if (!Number.isInteger(nextVersion) || nextVersion < 0 || nextVersion > 0x7fffffff) {
      throw new Error(`Invalid schema version: ${nextVersion}`);
    }
