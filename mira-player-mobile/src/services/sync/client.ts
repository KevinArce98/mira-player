...
async function request<T>(path: string, init: RequestInit, secret?: string): Promise<T> {
  ...
  const res = await fetch(`${SYNC_BASE_URL}${path}`, { ...init, headers: { ...headers, ...(init.headers ?? {}) } });
  if (!res.ok) {
...
export function pushProfile(secret: string, id: string, nombre: string, extra: Record<string, unknown> = {}) {
  return request<{ profile: unknown }>('/profiles', {
    method: 'POST',
    body: JSON.stringify({ id, nombre, ...extra }),
  }, secret);
}
...
export function deleteProfileRemote(secret: string, id: string) {
  return request<{ ok: boolean }>(`/profiles/${encodeURIComponent(id)}`, { method: 'DELETE' }, secret);
}

export function push(secret: string, body: PushBody): Promise<{ cursor: number }> {
  return request<{ cursor: number }>('/sync/push', { method: 'POST', body: JSON.stringify(body) }, secret);
}
