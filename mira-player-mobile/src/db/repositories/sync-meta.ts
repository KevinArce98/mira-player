...
const KEY_STALLED_PREFIX = 'sync_stalled_since_';
...
export async function getStalledSince(profileId: string): Promise<number | null> {
  const v = await getSyncState(KEY_STALLED_PREFIX + profileId);
  return v ? Number(v) : null;
}
export const setStalledSince = (profileId: string, ms: number | null) =>
  setSyncState(KEY_STALLED_PREFIX + profileId, ms === null ? null : String(ms));
...
export async function rekeyDataToProfile(profileId: string): Promise<void> {
  ...
  const now = Date.now();
  await db.runAsync(
    'UPDATE OR REPLACE progreso SET profile_id = ?, updated_at = COALESCE(updated_at, last_watched_at, ?), dirty = 1 WHERE profile_id IS NULL OR profile_id != ?;',
    [profileId, now, profileId],
  );
  await db.runAsync(
    'UPDATE OR REPLACE favoritos SET profile_id = ?, updated_at = COALESCE(updated_at, created_at, ?), dirty = 1 WHERE profile_id IS NULL OR profile_id != ?;',
    [profileId, now, profileId],
  );
}
...
export async function markAllDirty(profileId: string): Promise<void> {
  ...
  const now = Date.now();
  await db.runAsync(
    'UPDATE OR REPLACE progreso SET profile_id = ?, updated_at = COALESCE(updated_at, last_watched_at, ?), dirty = 1 WHERE profile_id = ? OR profile_id IS NULL;',
    [profileId, now, profileId],
  );
  await db.runAsync(
    'UPDATE OR REPLACE favoritos SET profile_id = ?, updated_at = COALESCE(updated_at, created_at, ?), dirty = 1 WHERE profile_id = ? OR profile_id IS NULL;',
    [profileId, now, profileId],
  );
}
