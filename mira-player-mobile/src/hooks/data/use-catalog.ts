...
export function useCatalog(
  ...
  return useQuery({
    queryKey: [...queryKeys.catalog(tipo, categoriaId, sort), fingerprint],
    queryFn: () =>
      queryContent({ cuentaId: cuentaId!, tipo, categoriaId, sort, limit: 500, parental: filter }),
...
export function useCatalogCount(
  ...
  const { filter, ready, fingerprint } = useParentalQuery();
  return useQuery({
    queryKey: [...queryKeys.catalogCount(tipo, categoriaId), fingerprint],
    queryFn: () => countContent({ cuentaId: cuentaId!, tipo, categoriaId, parental: filter }),
    enabled: !!cuentaId && ready,
  });
}
